...
# Para llamar al output del modulo de SubRed, se extrajo desde el archivo tf.state del modulo
data "terraform_remote_state" "subred_soc_primaria" {
  backend = "local"                                                               
  config = {
    path = "../SubRed/terraform.tfstate"  # ruta hacia el estado de desplieuge terraform de la red
  }
}

# Para llamar al output del modulo de la SubRed secundaria, se extrajo desde el archivo tf.state del modulo
data "terraform_remote_state" "subred_soc_secundaria" {
  backend = "local"
  config = {
    path = "../SubRed/terraform.tfstate"  # ruta hacia el estado de desplieuge terraform de la red
  }
...
locals {
  ...
  EMAIL_CUENTA_DE_SERVICIO = regex("(?m)^EMAIL_CUENTA_DE_SERVICIO=(.*)$", local.variables_content) != null ? regex("(?m)^EMAIL_CUENTA_DE_SERVICIO=(.*)$", local.variables_content)[0] : ""
}
